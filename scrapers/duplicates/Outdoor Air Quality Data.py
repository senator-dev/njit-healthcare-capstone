...
def download_one(pollutant, year, cbsa, driver):
    ...
    spark.sql(f"""
        insert into default.air_quality
...
# MAGIC %md
# MAGIC %sql
# MAGIC drop table if exists !!!!air_quality!!!!;
# MAGIC
...
data = pd.DataFrame(list(product(pollutant_values.keys(), years, cbsa_name_values.keys())), columns=['pollutant', 'year', 'cbsa'])
...
for flag, (i, row) in enumerate(data.iterrows()):
    ...
    try:
        response = download_one(row['pollutant'], row['year'], row['cbsa'], driver)
